...
class FRDGeneratorService:
    ...
    def _build_full_frd_prompt(self, work_item_id, context, combined_source):
        return f"""
    You are a senior functional consultant.
...
""",
    ...
    def _build_docx(
        ...
        if "full_frd" in sections:
            doc.add_heading("FRD Content", level=1)
            self._add_markdownish_content(doc, sections["full_frd"])
            doc.add_paragraph("")
        else:
            ordered_sections = [
                ("1. Overview", sections.get("overview", "")),
                ("2. Document History", sections.get("document_history", "")),
                ("3. Current State / Status Quo", sections.get("current_state", "")),
                (
                    "4. Proposed / Requested Solution",
                    sections.get("proposed_solution", ""),
                ),
                ("4.1 Roles", sections.get("roles", "")),
                ("4.2 Type of Applications", sections.get("application_types", "")),
                (
                    "4.3 Modules and Applications",
                    sections.get("modules_and_applications", ""),
                ),
                ("4.4 Process Flows", sections.get("process_flows", "")),
                (
                    "4.5 Functional Requirements",
                    sections.get("functional_requirements", ""),
                ),
                (
                    "4.6 Non-Functional Requirements",
                    sections.get("non_functional_requirements", ""),
                ),
                ("4.7 Integrations", sections.get("integrations", "")),
                ("4.8 Notifications", sections.get("notifications", "")),
                (
                    "4.9 Reporting / Visibility",
                    sections.get("reporting_visibility", ""),
                ),
                ("4.10 Gap Analysis", sections.get("gap_analysis", "")),
                ("4.11 Out of Scope", sections.get("out_of_scope", "")),
                (
                    "4.12 Assumptions and Constraints",
                    sections.get("assumptions_constraints", ""),
                ),
                ("4.13 Acceptance / Sign-off", sections.get("acceptance_signoff", "")),
            ]
